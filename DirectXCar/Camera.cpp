...
void XM_CALLCONV FirstPersonCamera::LookTo(DirectX::FXMVECTOR pos, DirectX::FXMVECTOR to, DirectX::FXMVECTOR up)
{
	...
	XMMATRIX R1 = XMMatrixRotationY(m_Theta);
	XMStoreFloat3(&m_Right, XMVector3TransformNormal(XMLoadFloat3(&m_Right), R1));
	XMStoreFloat3(&m_Up, XMVector3TransformNormal(XMLoadFloat3(&m_Up), R1));
	XMStoreFloat3(&m_Look, XMVector3TransformNormal(XMLoadFloat3(&m_Look), R1));
}
...
{
	m_Phi += rad;
	XMMATRIX R = XMMatrixRotationAxis(XMLoadFloat3(&m_Right), m_Phi);
...
{
	m_Theta += rad;
	XMMATRIX R = XMMatrixRotationY(m_Theta);
